...
// File: C:\Projects\Ion\Ion\Renderer\StorageBufferObject.cpp
...
ionBool StorageBuffer::Alloc(const VkDevice& _device, const void* _data, ionSize _allocSize, EBufferUsage _usage)
{
...

    {
        ...
        createInfo.m_type = EGpuMemoryType_Buffer;  // should be ok keep the same gpu granularity conflict check
...
void StorageBuffer::ReferenceTo(const StorageBuffer& _other)
{
...
void StorageBuffer::ReferenceTo(const StorageBuffer& _other, ionSize _refOffset, ionSize _refSize)
{
    ionAssertReturnVoid(!IsMapped(), "Buffer must be not mapped");
...
void StorageBuffer::Update(const void* _data, ionSize _size, ionSize _offset /*= 0*/) const
{
...
void* StorageBuffer::MapBuffer(EBufferMappingType _mapType)
{
    return MapBuffer(_mapType, 0);
}

void* StorageBuffer::MapBuffer(EBufferMappingType _mapType, ionSize _offset)
{
...
void StorageBuffer::ClearWithoutFreeing()
{
    m_size = 0;
